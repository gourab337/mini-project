...
static const std::set< std::string > PARAMS( params, params+5 );
...
public:
    ...
    std::string  m_strCamSerId;
    const char  *m_bookmarkText;
    FILETIME     m_time;
    unsigned     m_securityLevel;
...
//!
//! IvAddBookmark <options> -nvr <nvrIP> -cam <CamSerId> 
//!
//! Options:
//!  "    -d <BookmarkText>    Specifies the bookmarktext"
//!  "    -t <TimeStamp>       Specifies the timestamp (UTC) of bookmark (optional)
//!  "    -sec <SecurityLevel> Specifies the security level of bookmark (optional - defaults to 1)
//! 
///////////////////////////////////////////////////////////////////////////////
...
int main( int argc, char *argv[] )
{
    ...
    if ( options.m_strNvrIpAddr.empty() || options.m_strCamSerId.empty() )
    {
        PrintUsage();
        return -1;
    }
    ...
    // bookmark number and text, and security level
    //
    int nResult = IvBind2AddBookmark2(
        options.m_strNvrIpAddr.c_str(),
        options.m_strCamSerId.c_str(),
        options.m_bookmarkText,
        &options.m_time,
        options.m_securityLevel
        );
...
m_securityLevel( 1 )
{
    ...
    while ( ++nArg < argc )
    {
        ...
        if ( pszArg[ 0 ] == '-' || pszArg[ 0 ] == '/' )
        {
            ...
            else if ( strcmp( &pszArg[ 1 ], "sec" ) == 0 )
            {
                nArg++;

                if ( nArg >= argc )
                {
                    std::cout << "-sec: Please specify the security level" << std::endl;
                    exit( 1 );
                }
                try
                {
                    m_securityLevel = std::stoul( argv[nArg] , 0 , 10 );
                } catch( std::exception& )
                {
                    exit( 1 );
                }
...
//! 
//! \param  fileTime    Timestamp argument
//!
...
static void PrintUsage()
{
    std::cout << "\nUsage: IvAddBookmark2 <options> -nvr <nvrIP> -cam <camSerId>" << std::endl;
    std::cout << "\nOptions:" << std::endl;
    ...
    std::cout << "\t\t\t(Default the current time is used) "<< std::endl;
    std::cout << "    -sec <SecurityLevel>      Specifies a security level for the bookmark " << std::endl;
    std::cout << std::endl;
}
